...
- **실제로 전송한 페이지만**(`sent_on`이 있는 것) 표시하고 발송일 최신순으로 정렬한다.
  발행만 하고 아직 안 보낸 페이지는 열람이 없는 게 당연해서, 같이 두면 0%가 섞여
  발송분끼리의 비교를 흐린다.
...
- 화면 상단에 접이식 안내(`<details>`)를 둔다. 내용은 아래 "정확도에 대한 전제" 참고.

## 정확도에 대한 전제

`readers`는 **실제 인원의 상한선**이다. 한 사람이 여러 명으로 쪼개지는 일은 흔하지만,
여러 사람이 한 명으로 합쳐지는 일은 기기를 공유할 때뿐이라 드물다.

- 같은 고객이 휴대폰과 PC로 각각 열면 2명으로 잡힌다.
- 카톡에서 읽고 나중에 다른 브라우저로 다시 열면 2명으로 잡힌다.
- 안드로이드 카톡은 앱 데이터 삭제 전까지 유지되지만, **iOS 카톡 인앱 브라우저는 스토리지
  보존이 보장되지 않는다.** 같은 사람의 재방문이 새 기기로 잡혀 인원이 부풀려진다.
- 스토리지 차단 기기는 방문 1건이 곧 1명이 된다.

오차 방향이 한쪽으로 일정하므로 절대값은 신뢰하기 어렵지만, 페이지 간·발송 간 비교는
유효하다.

화면은 이 전제를 숨기지 않는다. `/admin/analytics` 상단에 접이식(`<details>`) 안내를 두어
"회와 명의 차이", "명은 사람이 아니라 기기라서 누구인지 알 수 없다", "인원은 실제보다 많게
나온다", "도달률 분모는 회다", "개인 확인은 개인별 링크가 있어야 한다"를 설명한다.
숫자만 보고 개인 단위로 오해하는 것이 이 화면에서 가장 비싼 오독이다.
